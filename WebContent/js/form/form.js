...
function drawTable(){
	Wheatmark.drawLineAnimate(20,100,20,1080);
	Wheatmark.drawLineAnimate(20,1080,773,1080);
	Wheatmark.drawLineAnimate(773,1080,773,100);
	Wheatmark.drawLineAnimate(773,100,20,100);
	Wheatmark.writeText('税务登记表（适用单位纳税人）',210,50,'30px 宋体 bold','start');
	Wheatmark.writeText('填表日期：'+getCurrentDate(),30,90,'20px 宋体','start');
	Wheatmark.drawLineAnimate(140,100,140,1080);
	Wheatmark.drawLineAnimate(400,100,400,1080);
	Wheatmark.drawLineAnimate(540,100,540,1080);
	var i,py;
	for(i=0;i<35;i++){
		py=100+28*(i+1);
		Wheatmark.drawLineAnimate(20,py,773,py);
	}
